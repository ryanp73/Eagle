...
{
	/**
	 * 		Key from The Blue Alliance API for this event 
	 *   	@var string
	 */
	public $eventKey;

	/**
	 *     Base part of the URL used to access the TBA API for this event
	 *     @var string
	 */
	private $_apiBase; 
	...
	public function __construct($eventKey)
	{
		...
		$this->_apiBase = "event/" . $this->eventKey;
		$this->loadBasicInfo();
	}
	...
	{
		$apiUrl = API_URL_BASE . $this->_apiBase . "/teams" . API_URL_ENDING;
		$json = file_get_contents($apiUrl);
		$data = json_decode($json);
	...
	{
		$apiUrl = API_URL_BASE . $this->_apiBase . "/matches" . API_URL_ENDING;
		$json = file_get_contents($apiUrl);
		$data = json_decode($json);
	...
	{
		$apiUrl = API_URL_BASE . $this->_apiBase . "/stats" . API_URL_ENDING;
		$json = file_get_contents($apiUrl);
		$data = json_decode($json);
